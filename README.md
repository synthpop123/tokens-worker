...
The two repos deploy independently, so a bump has no atomic moment — whichever ships first talks to the other side's previous version for a minute or two, and visitors without a session cache see the fallback for that window. To close it, temporarily let the consumer's `isSite` accept the previous version alongside the new one, **ship the homepage first**, then this Worker, then drop the old version again. At rest the consumer accepts exactly one version, which is what keeps the two sides honest; widening it is a deploy manoeuvre, not a standing allowance. It also only works for shapes the older reader survives — for a genuinely breaking change, the fallback window is the price.
