...
|---|---|
| `GET /api/site` | Precomposed dashboard view, one request for the whole page: per-range (`week`/`month`/`quarter`/`all`) totals + breakdowns (every row carries its usage span — `days`, `firstDate`, `lastDate`), the full daily series split **by provider and by client** with per-day `active` time, and the device inventory incl. CLI version, sessions, active-time metrics and MCP servers. No filters; served from KV. The body carries `schemaVersion` — see [Cross-repo contract](#cross-repo-contract) |
